...
// 주간 베스트 셀러
    ...
    $.ajax({
        ...
        data: {query:"과학",size:25},
        headers:{Authorization: "KakaoAK " + REST_API_KEY}
        
    }).done(function(bookdata){

        $("#weekendBooks").append('<div class="bookSlide slide3" style="display: none;"><div class="wrap"></div></div>');    // 카테고리별 슬라이드 묶음
        ...
        for(let i in bookdata.documents){
            ...
            if(i%5 == 0){
                $('#weekendBooks > .slide3 > .wrap').append('<div class="booksWrap"></div>')     // 5개씩 책 묶음
            }

            let booksWrap = $('#weekendBooks > .slide3 > .wrap > .booksWrap ').eq(index)
