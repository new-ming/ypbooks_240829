...
// 주간 베스트 셀러
    ...
    $.ajax({
        ...
        data: {query:"에세이",size:25},
        headers:{Authorization: "KakaoAK " + REST_API_KEY}
        
    }).done(function(bookdata){

        $("#newBooks").append('<div class="bookSlide slide1" style="display: none;"><div class="wrap"></div></div>');    // 카테고리별 슬라이드 묶음
        ...
        for(let i in bookdata.documents){
            ...
            if(i%5 == 0){
                $('#newBooks > .slide1 > .wrap').append('<div class="booksWrap"></div>')     // 5개씩 책 묶음
            }

            let booksWrap = $('#newBooks > .slide1 > .wrap > .booksWrap ').eq(index)
